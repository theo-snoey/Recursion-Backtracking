...
int countZeroSumSubsets(Vector<int> &v, int index, int sumSoFar) {
    if (index == v.size()) {
        ...
        int with = countZeroSumSubsets(v, index + 1, sumSoFar + v[index]);
        int without = countZeroSumSubsets(v, index + 1, sumSoFar);
        return with + without;
    }
}
...
int buggyCount(Vector<int> &v, int index, int sumSoFar) {
    if (index == v.size()) {
        ...
        int with = buggyCount(v, index + 1, sumSoFar + v[index]);
        int without = buggyCount(v, index + 1, sumSoFar);
        return with + without;
    }
}
...
STUDENT_TEST("testing my buggyCount function") {
    Vector<int> v = {1};

    EXPECT_EQUAL(countZeroSumSubsets(v, 0, 0), buggyCount(v, 0, 0));
}

STUDENT_TEST("testing my buggyCount function") {
    Vector<int> v = {3, 4, -4, 5, -1};
